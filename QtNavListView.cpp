...
void QtNavListModel::setItems(const QStringList &listItem)
{
    ...
    for (int i = 0; i < count; ++i) {
        const QString &item = listItem.at(i);
        QStringList list = item.split("|");
        if (list.count() < 4) {
            continue;
        }

        const QString &title = list.at(0);
        const QString &fatherTitle = list.at(1);
        const QString &collapse = list.at(2);
        const QString &info = list.at(3);
        ...
        if (fatherTitle.isEmpty()) {
            ...
            for (int j = 0; j < count; ++j) {
                const QString &secItem = listItem.at(j);
                QStringList secList = secItem.split("|");
                if (secList.count() < 4) {
                    continue;
                }

                const QString &secTitle = secList.at(0);
                const QString &secFatherTitle = secList.at(1);
                const QString &secInfo = secList.at(3);
